...
FixQEqReaxFixq::FixQEqReaxFixq(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg), pertype_option(nullptr)
{
  ...
  nca= utils::numeric(FLERR,arg[8],false,lmp);
  ca_charge= utils::numeric(FLERR,arg[9],false,lmp);
...
void FixQEqReaxFixq::post_constructor()
{
...
void FixQEqReaxFixq::pertype_parameters(char *arg)
{
...
void FixQEqReaxFixq::allocate_storage()
{
  nmax = atom->nmax;
...
void FixQEqReaxFixq::deallocate_storage()
{
  memory->destroy(s);
  memory->destroy(t);
...
void FixQEqReaxFixq::reallocate_storage()
{
  deallocate_storage();
  allocate_storage();
  init_storage();
...
void FixQEqReaxFixq::allocate_matrix()
{
  int i,ii,n,m;
...
void FixQEqReaxFixq::deallocate_matrix()
{
...
void FixQEqReaxFixq::reallocate_matrix()
{
  deallocate_matrix();
  allocate_matrix();
}
...
void FixQEqReaxFixq::init_list(int /*id*/, NeighList *ptr)
{
  list = ptr;
}
...
void FixQEqReaxFixq::init_shielding()
{
  int i,j;
  int ntypes;
...
void FixQEqReaxFixq::init_taper()
{
...
void FixQEqReaxFixq::setup_pre_force(int vflag)
{
  if (reaxc) {
    nn = reaxc->list->inum;
...
void FixQEqReaxFixq::setup_pre_force_respa(int vflag, int ilevel)
{
...
void FixQEqReaxFixq::min_setup_pre_force(int vflag)
{
  setup_pre_force(vflag);
}
...
void FixQEqReaxFixq::init_storage()
{
  int NN;
  int *ilist;
...
void FixQEqReaxFixq::pre_force(int /*vflag*/)
{
  ...
  init_matvec();
 // The new function CGP is used here 
  matvecs_s = CGP(b_s, s);       // CG on s - parallel
...
void FixQEqReaxFixq::pre_force_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixQEqReaxFixq::min_pre_force(int vflag)
{
  pre_force(vflag);
}
...
void FixQEqReaxFixq::init_matvec()
{
  /* fill-in H matrix */
  compute_H();

  int ii, i;
// The initial matrix values only for atom types other than 9. Atom type 9 is not part of the matrix
  for (ii = 0; ii < nn; ++ii) {
    i = ilist[ii];
    if (atom->mask[i] & groupbit) {
      if (atom->type[i]!=9) {
...
double FixQEqReaxFixq::calculate_H( double r, double gamma)
{
  double Taper, denom;
...
int FixQEqReaxFixq::CG( double *b, double *x)
{
  int  i, j;
...
int FixQEqReaxFixq::CGP( double *b, double *x)
{
  int  i, j;
  double tmp, alpha, beta, b_norm;
  double sig_old, sig_new;
...
void FixQEqReaxFixq::sparse_matvec( sparse_matrix *A, double *x, double *b, double *y )
{
  int i, j, itr_j;
  int ii;
// this function is modified so that the coulomb energy involving atom type 9 is deducted from b values
  for (ii = 0; ii < nn; ++ii) {
    i = ilist[ii];
    if (atom->mask[i] & groupbit)
  ...
  for (ii = 0; ii < nn; ++ii) {
    ...
    if (atom->mask[i] & groupbit) {
      for (itr_j=A->firstnbr[i]; itr_j<A->firstnbr[i]+A->numnbrs[i]; itr_j++) {
        j = A->jlist[itr_j];
        if (atom->type[i]==9 & atom->type[j]==9) continue;
        if (atom->type[i]==9 & atom->type[j]!=9) {
          b[j] += A->val[itr_j] * ca_charge;
        }
        else if (atom->type[j]==9 & atom->type[i]!=9) {
          b[i] += A->val[itr_j] * ca_charge;
        }
        else {
        b[i] += A->val[itr_j] * x[j];
        b[j] += A->val[itr_j] * x[i];
...
void FixQEqReaxFixq::sparse_matvec( sparse_matrix *A, double *x, double *b)
{
  int i, j, itr_j;
  int ii;
// If atom type i or atom type j are equal to 9, b is kept zero
  for (ii = 0; ii < nn; ++ii) {
    i = ilist[ii];
    if (atom->mask[i] & groupbit)
      b[i] = eta[ atom->type[i] ] * x[i];
...
void FixQEqReaxFixq::calculate_Q()
{
  int i, k;
  ...
  int ii;
// modify for the sum of charges through adding the charge of atom type 9
  s_sum = parallel_vector_acc( s, nn);
  t_sum = parallel_vector_acc( t, nn);
  u = (s_sum+ca_charge*nca) / t_sum;
// the charge on atom type 9 is predefined in the input fule 
  for (ii = 0; ii < nn; ++ii) {
    i = ilist[ii];
    if (atom->mask[i] & groupbit) {
      q[i] = s[i] - u * t[i];
...
int FixQEqReaxFixq::pack_forward_comm(int n, int *list, double *buf,
...
void FixQEqReaxFixq::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m;
...
int FixQEqReaxFixq::pack_reverse_comm(int n, int first, double *buf)
{
  int i, m;
  if (pack_flag == 5) {
    m = 0;
...
void FixQEqReaxFixq::unpack_reverse_comm(int n, int *list, double *buf)
{
  if (pack_flag == 5) {
    int m = 0;
    for(int i = 0; i < n; i++) {
...
double FixQEqReaxFixq::memory_usage()
{
  double bytes;
...
void FixQEqReaxFixq::grow_arrays(int nmax)
{
...
void FixQEqReaxFixq::copy_arrays(int i, int j, int /*delflag*/)
{
...
int FixQEqReaxFixq::pack_exchange(int i, double *buf)
{
...
int FixQEqReaxFixq::unpack_exchange(int nlocal, double *buf)
{
...
double FixQEqReaxFixq::parallel_norm( double *v, int n)
{
  int  i;
...
double FixQEqReaxFixq::parallel_dot( double *v1, double *v2, int n)
{
  int  i;
  double my_dot, res;
...
double FixQEqReaxFixq::parallel_vector_acc( double *v, int n)
{
  int  i;
  double my_acc, res;
...
void FixQEqReaxFixq::vector_sum( double* dest, double c, double* v,
...
void FixQEqReaxFixq::vector_add( double* dest, double c, double* v, int k)
{
  int kk;
